...
log_msg "Starting Gadang train_pix2pix DreamBooth grid"
...
for pair_count in $PAIR_COUNTS; do
  for learning_rate in $LEARNING_RATES; do
    for scope in $DREAMBOOTH_SCOPES; do
      ...
      run_name="${run_stamp}_train_pix2pix_dreambooth_pairs${pair_count}_lr${lr_slug}_scope${scope_slug}_steps${MAX_TRAIN_STEPS}"
      ...
      : > "$ROOT_LOG"
      log_msg "Running train_pix2pix DreamBooth run: $run_name"
      ...
      (
        ...
        uv run jupyter nbconvert \
          --to notebook \
          --execute notebook/instructpix2pix_dreambooth_grid.ipynb \
          --output "${executed_notebook}" \
          --ExecutePreprocessor.timeout=-1
      ) > >(tee "$log_path" >> "$ROOT_LOG") 2> >(tee -a "$log_path" >> "$ROOT_LOG")

      log_msg "Finished train_pix2pix DreamBooth run: $run_name"
      log_msg "Executed notebook: $executed_notebook"
    done
  done
done

log_msg "All requested Gadang train_pix2pix DreamBooth grid runs finished."
